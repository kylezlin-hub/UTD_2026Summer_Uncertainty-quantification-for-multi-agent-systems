...
def build_turn_prompt(
    ...
    agent_previous = (
        {
            "answer": clean_text(row.get(f"R{previous_round} {agent} Answer")),
            "explanation": clean_text(row.get(f"R{previous_round} {agent} Response")),
        }
        if previous_round >= 1
        else {}
    )
    ...
    if previous_round >= 1:
        for other in agents:
            ...
            other_previous.append(
                {
                    "agent": other,
                    "answer": clean_text(row.get(f"R{previous_round} {other} Answer")),
                    "explanation": clean_text(row.get(f"R{previous_round} {other} Response")),
                }
            )
    payload = {
        "source_file": source_file,
        "question_no": row.get("Question #"),
        "question_or_scenario": first_existing(row, ["Question", "Scenario", "Prompt"]),
        "round": round_no,
        "agent": agent,
...
def main() -> None:
    ...
    if args.limit is not None:
        limited_path = args.out_dir / f"{args.input.stem}.limited_{args.limit}.xlsx"
